const int CURRENTSENSE = A3;   // Pin for current sensing
const int VOLTSENSE = A4;      // Pin for voltage sensing
const int numSamples = 1000;     // Number of samples to store
...
const float CURRENTNORM = 0.0144; // Normalizing constant for current 
const float VREF = 1876;         // Reference voltage for current sensing(value had to be calibrated , initial value was 498, connect killa-watt ,without load ad calibrate current to approx 0)
#define POWERFACTOR  1
void setup() {
  Serial.begin(115200);  // Initialize serial communication at 115200 baud
  // analogReadResolution(12);  // Set ADC resolution to 10 bits
...
void loop() {
  ...
  for (int i = 0; i < numSamples; i++) {
    wattdata[i] = voltageSamples[i] * currentSamples[i];
  }

  float avgAmp=0,avgWatt = 0;
  for (int i = 0; i < numSamples; i++) {
    avgAmp += abs(currentSamples[i]);
...
//send time placeholder
  ...
  //  Serial.print(",");
  // Serial.print(rmsVoltage*rmsCurrent*POWERFACTOR);
